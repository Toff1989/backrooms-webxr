import { CELL_SIZE, PLAYER_SPRINT_SPEED } from "../../src/shared/constants.js";
 ...
 * Le temps minimum plausible pour la traverser est `distance / PLAYER_SPRINT_SPEED` (vitesse
 * max, sprint compris) : c'est une borne physique absolue (le vrai trajet dans un
 * labyrinthe ne peut être QUE plus long que la ligne droite, jamais plus court), donc un
 * temps mesuré en dessous est matériellement impossible.
 */
export function computeMinPlausibleMillis(seed: string, fromDepth: number): number {
  ...
  return (distance / PLAYER_SPRINT_SPEED) * 1000;
}
