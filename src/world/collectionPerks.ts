 ...
 * - supports vidéo (cassette, caméscope, caméra de surveillance) : la corruption visuelle se
 *   dissipe 15 % plus vite chacun (plafond +75 %) — "têtes de lecture propres" ;
...
export function computePerks(entries: readonly CollectionEntry[]): CollectionPerks {
  ...
  const batteryCapacity = Math.min(2.5, 1 + count(["flashlight", "vintageFlashlight"]) * 0.25 + count(["lighter"]) * 0.1 + entries.length * 0.01);
  const corruptionDecay = Math.min(1.75, 1 + count(["tape", "videoCamera", "securityCamera"]) * 0.15);
